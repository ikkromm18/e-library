<div wire:preserveScroll>
    ...
    @if ($showForm)
        <div class="bg-surface rounded-lg shadow-sm p-6 mb-6">
            <h2 class="text-lg font-semibold text-text-primary mb-4">{{ $editId ? 'Edit Kategori' : 'Tambah Kategori' }}</h2>
            <form wire:submit="save">
                <div class="max-w-md">
                    <label class="block text-sm font-medium text-text-primary">Nama Kategori</label>
    ...
    <div class="bg-surface border border-border rounded-lg shadow-sm overflow-hidden">
        <table class="min-w-full divide-y divide-border">
            ...
            <tbody class="bg-surface divide-y divide-border">
                ...
                @empty
                    <tr><td colspan="4" class="px-6 py-4 text-center text-sm text-text-secondary">Belum ada kategori.</td></tr>
                @endforelse
            </tbody>
        </table>
